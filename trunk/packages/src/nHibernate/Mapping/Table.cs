	...
	public class Table : IRelationalModel
	{
		...
		public string SqlCreateString(Dialect.Dialect dialect, IMapping p, string defaultCatalog, string defaultSchema)
		{
			...
			if (HasPrimaryKey && (dialect.GenerateTablePrimaryKeyConstraintForIdentityColumn || !identityColumn) 
                && (!dialect.SupportsPrimaryKeyConstraintInAlterTable || String.IsNullOrEmpty(PrimaryKeyName))) {
		...
		{
            var dropString = dialect.GetDropTableString(GetQualifiedName(dialect, defaultCatalog, defaultSchema));
            if(!String.IsNullOrEmpty(catalog)) {
                dropString = string.Format("use {0} {1}", catalog, dropString);
            }
		    return dropString;
		}
